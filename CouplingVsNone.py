...
def run_simulation_4agents(
    ...
    for n in range(max_delay, N + max_delay - 1):
        ...
        for agent_idx, agent in enumerate(agents):
            ...
            if agent.coupling_row is not None:
...
def simulate_run_corrected(args):
    config_name, tau, params = args
    agents = params['agents_configs'][config_name]
    _, data = simulate_tau(
        tau=tau,
        varying_tau=params['varying_tau'],
        fixed_delays=params['fixed_delays'],
        agents=agents,
    ...
    return config_name, tau, data


def vary_and_plot_4agents(params, coupling_configs):
    ...
    results = {config_name: {} for config_name in coupling_configs.keys()}

    agents_configs = {}
    for config_name, coupling_enabled in coupling_configs.items():
        if coupling_enabled:
            agents_configs[config_name] = agents_template
        else:
            agents_no_coupling = [
                Agent(
                    a_x=agent.a_x,
                    c_x1=agent.c_x[0], c_x2=agent.c_x[1],
                    c_x3=agent.c_x[2], c_x4=agent.c_x[3],
                    c_x12=agent.c_x12, c_x13=agent.c_x13,
                    c_x14=agent.c_x14, c_x23=agent.c_x23,
                    c_x24=agent.c_x24, c_x34=agent.c_x34,
                    c_x123=agent.c_x123, c_x124=agent.c_x124,
                    c_x134=agent.c_x134, c_x234=agent.c_x234,
                    pop_ratio=agent.pop_ratio,
                    coupling_row=None  
                ) for agent in agents_template
            ]
            agents_configs[config_name] = agents_no_coupling

    tasks = []
    for config_name in coupling_configs.keys():
        for tau in tau_values:
            tasks.append( (config_name, tau, {
    ...
    with mp.Pool(mp.cpu_count()) as pool:
        results_list = pool.map(simulate_run_corrected, tasks)

    for config_name in coupling_configs.keys():
        results[config_name] = {}

    for res in results_list:
        config_name, tau, data = res
        results[config_name][tau] = data

    plt.figure(figsize=(14,8))
    for config_name in coupling_configs.keys():
        mu_idx = mu_labels.index("mu1111")
        steady_probs = [results[config_name][t]['steady'][mu_idx] for t in tau_values]
        plt.plot(tau_values, steady_probs, label=config_name)

    plt.title(f"Probability of mu1111 vs Varying Tau ({varying_tau}) [Steady Portion Only]")
    plt.xlabel(varying_tau)
    plt.ylabel("Probability of mu1111")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.show()
...
def main():
    ...
    lambda_params = np.array([0.3, 0.7, 0.5, 0.9], dtype=np.float64)
    sigma_params  = np.array([0.003, 0.006, 0.005, 0.01], dtype=np.float64)
    coupling_configs = {
        "With Coupling Run 1": True,
        "With Coupling Run 2": True,
        "With Coupling Run 3": True,
        "No Coupling": False
    }
    ...
    for varying_tau, fixed_tau_dict in sweeps:
        ...
        vary_and_plot_4agents(params, coupling_configs)
